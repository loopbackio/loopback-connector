...
SQLConnector.prototype.buildUpdate = function(model, where, data, options) {
  ...
  var whereClause = this.buildWhere(model, where, true);
 ...
 * @returns {array} Array of relations to make joins
 */
SQLConnector.prototype._innerRelations = function(model, where) {
  var result = [];
  ...
  if (relations) {
    for (var key in where) {
      if (key in relations) {
        ...
        result.push(rel);
        result = result.concat(this._innerRelations(modelTo, where[key]));
      }
    }
  }
  return result;
...
/**
 * Build the outer joins for the where object
 * @param {string} model Model name
 * @param {object|array|string} order order by clouse
 * @returns {array} Array of relations
 */
SQLConnector.prototype._outerRelations = function(model, orders) {
  var result = [];
    ...
    if (typeof order === 'object') {
      for (var key in order) {
        if (key in relations) {
          ...
          result.push(rel);
          var modelTo = rel.modelTo.definition.name;
          result = result.concat(self._outerRelations(modelTo, order[key]));
        }
      }
    }
  }
...
SQLConnector.prototype.buildJoins = function(model, where, order) {
  ...
  function makeJoinStmt(isInner, rel) {
    var joinType = isInner ? 'INNER JOIN ' : 'LEFT OUTER JOIN ';
...
SQLConnector.prototype._buildWhere = function(model, where, qualified) {
  ...
  for (var key in where) {
    ...
    if (relations && key in relations) {
      var stmtForRelation = self._buildWhere(key, where[key], qualified);
...
SQLConnector.prototype.buildFieldsForUpdate = function(model, data, excludeIds) {
  ...
  var fields = this.buildFields(model, data, excludeIds, true);
