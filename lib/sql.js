        ...
        for (var i = 0, n = clauses.length; i < n; i++) {
          ...
          for (var ck in clauses[i]) {
            if (ck in relations) {
              ...
              var relClauses = [];
              var relModel = relations[ck].modelTo;
              ...
              do {
                ...
                for (var wk in lastRelWhere) {
                  if (wk in relModel.relations) {
                    relModel = relModel.relations[wk].modelTo;
                    relWhere = lastRelWhere[wk];
                  } else {
                    var w = {};
                    w[wk] = lastRelWhere[wk];
                    relClauses.push(self._buildWhere(loopRelModel.definition.name, w));
                  }
                }
  ...
  for (var i = 0, n = order.length; i < n; i++) {
    ...
    if (t[0].indexOf('.') < 0) {
      ...
      for (var y = 0; y < colSplit.length; y++) {
        ...
        prevModel = colModel;
        // ... and escape them
        colName = self.columnEscaped(colModel, colSplit[colSplit.length - 1]);
      }
    }
    ...
    if (Array.isArray(where)) {
      ...
      for (var key in where) {
        if (key === 'and' || key === 'or') {
          ...
          dest[key] = dest[key] || {};
          dest[key].where = dest[key].where || {};
          fixWhere(where[key], relations[key].modelTo.relations, dest[key].where);
        }
      }
    }
  }
...
SQLConnector.prototype.buildJoins = function(model, where) {
  ...
  var buildOneToMany = function buildOneToMany(modelFrom, keyFrom, modelTo, keyTo, joinWhere) {
    var modelToEscaped = this.tableEscaped(modelTo);
    var relModel = this.getModelDefinition(modelTo);
    var relRelations = relModel.model.relations;
    var joinType = 'LEFT';

    var condition = new ParameterizedSQL(
      'ON ' +
      this.columnEscaped(modelFrom, keyFrom) + '=' +
    ...
    if (joinWhere) {
      ...
      var innerWhere = this._buildWhere(modelTo, joinWhere);
      if (innerWhere.sql.length) {
        condition.merge(['AND', innerWhere]);
      }
    }

    return new ParameterizedSQL(joinType + ' JOIN ' + modelToEscaped, [])
      .merge(condition)
      .merge(this.buildJoins(modelTo, joinWhere));
  }.bind(this);
  ...
  for (var key in where) {
    ...
    if (!rel.modelThrough) {
      // 1:n relation
      join = buildOneToMany(model, keyFrom, modelTo, keyTo, where[key].where);
    } else {
      // n:m relation
      ...
      var joinInner = buildOneToMany(model, keyFrom, modelThrough, keyTo);
      join = buildOneToMany(modelThrough, keyThrough, modelTo, modelToKey, where[key].where);
