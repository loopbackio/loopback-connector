      ...
      if (operator === 'inq' || operator === 'nin' || operator === 'between') {
        ...
        if (Array.isArray(expression)) {
          ...
          for (var j = 0, m = expression.length; j < m; j++) {
            ...
            if (columnValue[j].sql && columnValue[j].sql.match(/DEFAULT\(\?\)/)) {
              columnValue[j].params.pop();
              columnValue[j].params.push(columnName);
            }
          }
        } else {
          columnValue.push(this.toColumnValue(p, expression));
          if (columnValue.sql && columnValue.sql.match(/DEFAULT\(\?\)/)) {
            columnValue.params.pop();
            columnValue.params.push(columnName);
          }
        }
        if (operator === 'between') {
          // BETWEEN v1 AND v2
