...
SQLConnector.prototype.buildJoins = function(model, where, prevRelName) {
  ...
  var buildOneToMany = function buildOneToMany(relName, modelFrom, keyFrom, modelTo, keyTo, joinWhere,
    ...
    var condition = new ParameterizedSQL(
      'ON ' +
      this.columnEscaped(modelFrom, keyFrom, forcePrevRelName || prevRelName) + '=' +
      this.columnEscaped(modelTo, keyTo, relName)
    );
    ...
    if (joinWhere) {
      ...
      var innerWhere = this._buildWhere(modelTo, joinWhere, relName);
    ...
    join += ' AS ' + this.escapeName(relName);

    return new ParameterizedSQL(join, [])
      .merge(condition)
      .merge(this.buildJoins(modelTo, joinWhere, relName));
  }.bind(this);
  ...
  for (var key in where) {
    ...
    if (!rel.modelThrough) {
      // 1:n relation
      join = buildOneToMany(relName, model, keyFrom, modelTo, keyTo, where[key].where);
    } else {
      // n:m relation
      ...
      join = buildOneToMany(relName, modelThrough, keyThrough, modelTo, modelToKey, where[key].where,
                            relName + '_though');
